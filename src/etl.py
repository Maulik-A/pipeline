...
def merge_to_fact_table(
    ...
    QUERY = f'''
        ...
        USING(
            ...
            FROM "{catalog}"."{database}"."{src_table}") src
        ...
        WHEN NOT MATCHED THEN INSERT 
            (pk_id, event_id, event_year, event_code, event_num, session_id, ts,drivernumber, rpm,speed, gear, throttle, breaks, drs)
        VALUES(src.pk_id, src.event_id, src.event_year, src.event_code, src.event_num, src.session_id,
            src.ts,src.drivernumber, src.rpm,src.speed, src.gear, src.throttle, src.breaks, src.drs)
        '''
    try:
